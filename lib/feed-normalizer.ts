...
export function formatPublishedAt(timestampMs: number): string {
  ...
  const todayStr = new Date().toLocaleDateString("en-US", { timeZone: LOCAL_TZ })
  ...
  const yesterdayStr = yesterday.toLocaleDateString("en-US", { timeZone: LOCAL_TZ })
...
export function normalizeStory(raw: RawStory, source: Source): NewsItem {
  ...
  return {
    ...
    publishedTimestamp,
    summaryEn: raw.description,
    summaryNp: "", // Populated by translation pipeline when connected
