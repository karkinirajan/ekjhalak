...
test("a Devanagari danda counts as a sentence boundary", () => {
    const np = "यो पहिलो वाक्य हो। ".repeat(30);
...
test("a sentence boundary too early in the window is not used", () => {
    // "Hi. " ends at character 3 — using it would return three characters where
    // 400 were available.
    const text = `Hi. ${"a".repeat(600)}`;
